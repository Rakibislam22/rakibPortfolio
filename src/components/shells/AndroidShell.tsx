...
export function AndroidShell({
  ...
  const getMaterialIcon = (id: AppId) => {
    switch (id) {
      case "about":
        return (
          <div className={`flex h-14 w-14 items-center justify-center rounded-full border-2 shadow-md transition ${
            isLight
      ...
      case "terminal":
        return (
          <div className={`flex h-14 w-14 items-center justify-center rounded-full border-2 font-mono text-lg font-bold shadow-md transition ${
            isLight
  ...
  return (
    <div className="relative h-full w-full flex flex-col justify-between overflow-hidden select-none">
      ...
      <header className={`fixed top-0 left-0 right-0 z-40 flex h-8 items-center justify-between px-5 text-xs ${
      ...
      <main className="flex-1 flex flex-col justify-between pt-12 pb-20 px-5 max-w-sm mx-auto w-full">
        ...
        <div className="w-full">
          <div
            ...
            className={`flex items-center justify-between rounded-full px-4 py-2.5 backdrop-blur-xl shadow-lg cursor-pointer transition border ${
              isLight
      ...
      {activeApp && (
        <div className={`fixed inset-0 z-50 flex flex-col animate-in fade-in duration-200 ${
          ...
          <div className={`flex items-center justify-between border-b px-4 py-3 backdrop-blur-xl ${
            isLight ? "border-slate-200 bg-white/95" : "border-white/10 bg-slate-900/90"
          }`}>
            <button
              onClick={() => onCloseApp(activeApp.id)}
              className={`flex items-center gap-2 text-xs font-semibold ${
                isLight ? "text-teal-700 hover:text-teal-900" : "text-teal-300 hover:text-teal-200"
              }`}
            >
